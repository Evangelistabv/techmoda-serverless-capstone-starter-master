...
## Entrega y evidencia del despliegue

Repositorio: [Evangelistabv/techmoda-serverless-capstone-starter-master](https://github.com/Evangelistabv/techmoda-serverless-capstone-starter-master).

Las URLs reales `ApiUrl` y `FrontendUrl` se documentan exclusivamente en
`private/DEPLOYMENT.private.md` (documento local excluido de Git). El README raíz
no contiene endpoints del despliegue. La API fue comprobada con HTTP 200 el
2026-09-24. El frontend fue proporcionado por el usuario y comprobado con HTTP 403;
su URL queda únicamente en el documento privado.

### Bedrock: modelos utilizados y motivos

| Modelo / perfil | Uso | Motivo en esta implementación | Evidencia |
|---|---|---|---|
| `us.anthropic.claude-haiku-4-5-20251001-v1:0` | S6 y S8 | Descripciones breves y conversación sobre catálogo con Converse; límites de 300/400 tokens de salida | Respuestas reales con `usage` |
| `amazon.titan-embed-text-v2:0` | S7 y recuperación de S8 | Vectores para búsqueda semántica y contexto del asistente | Cuatro productos indexados en S7 |

La nota de S11 confirma acceso efectivo a esos modelos durante la demo; no registra
la fecha de habilitación. La configuración local usa `us-east-1`.

### Tokens y control de costos (D5)

| Sesión de la demo S11 | Entrada | Salida | Total |
|---|---:|---:|---:|
| S6 | 175 | 105 | 280 |
| S8 | 276 | 114 | 390 |
| **Total** | **451** | **219** | **670** |

Ver los [extractos JSON y su procedencia](docs/evidence/README.md), incluida una
comprobación adicional de S8 del 2026-09-24. Los tokens de embeddings y otros
servicios no están incluidos en esos totales; no equivalen al costo total de AWS.

Para desplegar en otra cuenta, crear la política `techmoda-capstone-boundary` y
configurar el parámetro `BedrockGuardrailIdParam` localmente si se usa un guardrail.
Los templates resuelven el ARN del boundary con la cuenta de destino; no incluyen
el identificador de la cuenta ni el guardrail del despliegue original.
...
Proyecto **standalone** (no forma parte de ningún monorepo). La publicación del código en GitHub
no despliega automáticamente recursos de AWS.
